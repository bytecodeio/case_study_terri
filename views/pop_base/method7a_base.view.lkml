...
# compare it to the previous period of the same length - great for single value visualization comparisons
    ...
    dimension_group: filter_start_date {
      ...
      sql: DATE_TRUNC(current_date(), {% date_filter._parameter_value  %}) ;;
      convert_tz: no
    }
...
#start date of the previous period
    dimension: previous_start_date {
      hidden: yes
      type: string
      sql: DATE_ADD(${filter_start_date_raw}, INTERVAL-1 {% date_filter._parameter_value %});;
    }
...
## For filtered measures
    ...
    dimension: is_previous_period {
      hidden: yes
      type: yesno
      sql: ${pop_date_field_date} >= ${previous_start_date} AND ${pop_date_field_date} < ${previous_end_date} ;;
    }
